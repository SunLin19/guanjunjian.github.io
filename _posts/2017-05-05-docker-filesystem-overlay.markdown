...
这个目录在路径/var/lib/docker/overlay下。硬链接被用来索引和低层共享的数据，节省了空间。

当创建一个容器时，overlay驱动连接代表镜像层顶层的目录和一个代表容器层的新目录。
...
此时，在路径/var/lib/docker/overlay/下，每个镜像层都有一个对应的目录。用来存储每个镜像层的内容。使用tree命令看看该目录下的内容。
...
镜像层目录包含了该层独有的文件以及和其低层共享的数据的硬连接：
...
”upper“目录是容器的读写层，任何容器做出的改变都被写到这个目录下。

“merged”目录是容器的实际挂载点，这里提供了镜像层和容器层的统一视图。
容器的任何改变都会立即反映在这个目录下。

OverlayFS要求"work"目录正常工作，它用于诸如copy-up之类的操作.
...
从第二层开始，每层镜像层包含”lower“文件，根据这个文件可以索引构建出层的构成。
”diff“文件(层的内容)。还包含”merged“和"work"目录。
...
再来看看容器层，容器层目录包含相似的文件和目录，使用刚刚拉取的ubuntu镜像创建一个容器，/var/lib/docker/overlay2目录下新增2个目录：
...
* ***读的文件不在容器层**：如果读的文件不在容器层，则从镜像层进行读。这带来的性能开销很小*
* ***读的文件只存在在容器层**：直接从容器层读*
...
OverlayFS能在大多数Linux文件系统上运行，但是ext4是目前在生产环境中被推荐的。
...
也可以通过修改docker config文件，增加--storage-driver=overlay到DOCKER_OPTS行使得docker daemon自动使用overlay/overlay启动

4). 核实daemon已经在使用overlay/overlay
```shell
$ docker info
